...
│                                                 │
│  1. ASR监听 → 捕获用户语音（ROS2）               │
│  2. 摄像头拍照 → 获取环境图像(RealSense)          │
│  3. HTTP POST → 发送到Thor服务器                 │
│  4. 同步等待 → 接收Thor JSON响应                 │
│  5. 执行动作 → 说话、手势、移动                  │
│                                                 │
└─────────────────────────────────────────────────┘
                    ↕️  HTTP/REST API
...
│                                                 │
│  1. Flask HTTP服务器 → 监听5000端口              │
│  2. 接收POST /infer → 文本 + 图像(Base64)        │
│  3. VLM推理 → Qwen2.5-VL模型处理                │
│  4. 决策生成 → 确定回复和动作                    │
│  5. 返回JSON → 同步响应给G1                      │
│                                                 │
└─────────────────────────────────────────────────┘
...
```
用户说话 → ASR识别(ROS2) → G1拍照 → 打包JSON
    ↓
HTTP POST 到 http://THOR_IP:5000/infer
    ↓
Thor接收 → VLM推理 → 生成响应JSON
    ↓
HTTP 200 OK + JSON响应体
    ↓
...
opencv-python >= 4.8.0
pyrealsense2 >= 2.50.0  # Intel RealSense D435i支持
numpy >= 1.24.0
```
...
# 应该看到G1头顶的D435i（序列号: 233722074381）
# 如果设备序列号不同，需要修改配置

# 测试RealSense摄像头
realsense-viewer  # 可选：打开可视化工具测试
```
...
# RealSense D435i配置（默认值适用于G1头顶摄像头）
REALSENSE_DEVICE_SN = "233722074381"  # RealSense设备序列号
CAMERA_WIDTH = 640       # 图像宽度
CAMERA_HEIGHT = 480      # 图像高度
CAMERA_FPS = 15          # 帧率（RealSense推荐15fps）

# Thor HTTP服务器配置
THOR_HOST = "192.168.10.20"  # Thor服务器IP地址
THOR_PORT = 5000             # Thor HTTP端口
THOR_URL = f"http://{THOR_HOST}:{THOR_PORT}"  # 完整URL
THOR_TIMEOUT = 30.0          # HTTP请求超时时间（秒）
...
# 修改RealSense设备序列号（如果有多个RealSense设备）
export G1_REALSENSE_SN=233722074381

# 修改语言
export G1_SPEAKER=0  # 中文

# 修改网络接口（也可以通过启动参数指定，仅用于ASR监听）
...
# 基本启动（假设ASR使用eth0，Thor在192.168.10.20:5000）
python -m g1_robot_controller eth0

# 自定义Thor地址启动
G1_THOR_URL=http://192.168.10.20:5000 python -m g1_robot_controller eth0
...
🔊 初始化文本转语音(TTS)...
📤 初始化Thor HTTP发送器...
📤 Thor HTTP sender initialized: http://192.168.10.20:5000
⚙️ 初始化响应分发器...
🎤 初始化语音识别(ASR)监听器...
============================================================
✅ 所有组件初始化成功！
...
├── thor_vlm_server.py              ← Thor端VLM推理HTTP服务
├── requirements_thor.txt           ← Thor端依赖
└── models/                         ← VLM模型目录
...
scp requirements_thor.txt nvidia@<Thor_IP>:/home/nvidia/thor_vlm_server/
```
...
# 安装依赖
pip install -r requirements_thor.txt -i https://pypi.tuna.tsinghua.edu.cn/simple
```
...
============================================================
🖥️  CUDA可用: True
🎮 GPU设备: NVIDIA Jetson Orin
💾 GPU内存: 32.00 GB
📂 模型路径: /home/bryce/models/Qwen2.5-VL-3B-Instruct
📐 图像尺寸: 420x280
============================================================
🚀 Thor VLM HTTP服务器 - 启动中
============================================================
📡 监听地址: 0.0.0.0:5000
🔗 推理端点: POST http://0.0.0.0:5000/infer
💚 健康检查: GET http://0.0.0.0:5000/health
⌨️  按 Ctrl+C 停止服务器
============================================================
 * Serving Flask app 'thor_vlm_server'
...
# 从G1上测试（替换THOR_IP）
curl http://192.168.10.20:5000/health

# 期望输出:
# {"status":"healthy","model_loaded":true,"model_path":"/home/bryce/models/Qwen2.5-VL-3B-Instruct"}
```
...
#### 症状
```
❌ 初始化失败: RuntimeError: No RealSense devices detected
```

#### 原因
RealSense摄像头未连接或驱动未安装
...
# 3. 重新安装RealSense驱动（如需要）
sudo apt-get install librealsense2-dkms librealsense2-utils

# 4. 如果设备序列号不同，修改环境变量
export G1_REALSENSE_SN=你的设备序列号
python -m g1_robot_controller eth0
```
...
### 问题5: RealSense摄像头采集失败

#### 症状
```
⚠️ Failed to capture frame from RealSense after 10 attempts
⚠️ RealSense camera capture error
```
...
# 应该看到设备序列号: 233722074381
```

**2. 验证设备序列号**
```bash
# 如果序列号不同，设置环境变量
export G1_REALSENSE_SN=你看到的序列号
python -m g1_robot_controller eth0
```
...
# 或运行简单测试
cd /home/unitree/project1/g1_agent/finall_g1_agent
python3 g1_camera_head.py
```

**4. 检查USB连接**
```bash
# 确认Intel RealSense设备已连接
lsusb | grep Intel

# 应该看到类似: Bus 001 Device 004: ID 8086:0b3a Intel Corp. RealSense D435i
```
...
- [ ] 网络接口名称已确认 (`ifconfig`)
- [ ] RealSense D435i摄像头可用 (`rs-enumerate-devices`)
- [ ] 配置文件 `g1_robot_controller/utils/config.py` 已检查
- [ ] RealSense设备序列号已确认（默认: 233722074381）
